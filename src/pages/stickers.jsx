      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).length === 0 && <p>Sorry All the Stickers are currently our of scope new stock coming soon. Stay Tuned!</p>}
            {Object.keys(products).map((item) => {
              return (
                <Link key={products[item]._id} passHref className="lg:w-1/5 md:w-1/2 p-4 w-full m-5 shadow-lg" href={`/product/${products[item].slug}`}>
  ...
  for (let item of products) {
    if (item.title in stickers) {
      if (!stickers[item.title].color.includes(item.color) && item.availableQty > 0) {
        stickers[item.title].color.push(item.color.toUpperCase());
      }
      if (!stickers[item.title].size.includes(item.size) && item.availableQty > 0) {
        stickers[item.title].size.push(item.size.toUpperCase());
      }
    } else {
      stickers[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        stickers[item.title].color = [item.color.toUpperCase()];
        stickers[item.title].size = [item.size.toUpperCase()];
      }
    }
  }
  return {
