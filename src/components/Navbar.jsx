...
const Navbar = ({ logout, user, cart, addToCart, removeFromCart, clearCart, subTotal }) => {
  ...
  useEffect(() => {
    ...
    if (["/checkout", "/orders", "/order", "/myaccount"].includes(router.pathname)) {
      setSidebar(false);
    }
  }, []);
  ...
  return (
    <>
      {!sidebar && (
        <span className="fixed cursor-pointer right-10 top-4 z-30" onMouseOver={() => setDropdown(true)} onMouseLeave={() => setDropdown(false)}>
          {user && user.value && <MdAccountCircle className="text-xl md:text-2xl mx-2" />}
          {dropdown && (
            <div className="absolute right-7 top-5 rounded-md px-5 w-36 z-20 bg-white shadow-lg border">
              <ul>
                <Link href={"/myaccount"}>
                  <li className="py-1 hover:text-pink-700 text-sm font-bold">My Account</li>
                </Link>
                <Link href={"/orders"}>
                  <li className="py-1 hover:text-pink-700 text-sm font-bold">My Orders</li>
                </Link>
      ...
      <div className={`flex sticky top-0 bg-white z-10 flex-col md:flex-row md:justify-start py-2 justify-center items-center shadow-md ${!sidebar && "overflow-hidden"}`}>
        ...
        <div ref={ref} className={`sideCart z-50 overflow-y-auto h-[100vh] w-72 fixed bg-pink-100 top-0 py-10 px-8 ${sidebar ? "right-0" : "-right-96"} transition-all`}>
